...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
	    ...
	    char buffer[50]; // Buffer to store the formatted text string
	    ...
	    sprintf(buffer, "%02d:%02d:%02d | 20%02d-%02d-%02d \r\n",
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN Check_RTC_BKUP */
	...
	if (prev == MAGIC_NUM) {
        if (btn != BOARD_KEY0_polarity) {	// "!= active_polarity" tj nije aktivno tj "nije pritisnuto"
